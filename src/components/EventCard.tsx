...
export function EventCard({ event }: EventCardProps) {
  ...
  return (
    <Link
      to={`/event/${event.id}`}
      className="data-card group block hover-lift card-glow-hover"
    >
      <div className="flex flex-col h-full">
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="flex-1 min-w-0">
            ...
            <h3 className="text-lg font-semibold text-foreground mt-1 line-clamp-2 group-hover:text-ocean-teal transition-colors">
              {event.title}
            </h3>
          </div>
          {event.ai_prediction && (
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div className="bg-background-tertiary/50 rounded-lg p-3">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-2xl font-bold text-ocean-seafoam">
                {aiYesProb.toFixed(0)}%
              </span>
              {event.ai_prediction && (
        ...
        <div className="flex items-center justify-between mt-auto pt-3 border-t border-border">
          ...
          <span className="text-ocean-teal text-sm flex items-center gap-1 group-hover:gap-2 transition-all">
